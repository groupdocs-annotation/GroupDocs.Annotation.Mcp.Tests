...
main() {
    echo ""
    echo "╔════════════════════════════════════════════════════╗"
    echo "║  GroupDocs.Annotation.Mcp — Setup Verification      ║"
    echo "╚════════════════════════════════════════════════════╝"
    echo ""
    ...
    # Check test project
    if [ -d "$PROJECT_ROOT/src/GroupDocs.Annotation.Mcp.Tests" ]; then
        print_pass "Test project found"
    else
        print_fail "Test project not found: $PROJECT_ROOT/src/GroupDocs.Annotation.Mcp.Tests"
    fi
    
    # Check csproj
    if [ -f "$PROJECT_ROOT/src/GroupDocs.Annotation.Mcp.Tests/GroupDocs.Annotation.Mcp.Tests.csproj" ]; then
    ...
    if [ -d "$PROJECT_ROOT/Files" ]; then
        local doc_count=$(find "$PROJECT_ROOT/Files" -type f 2>/dev/null | wc -l)
        if [ "$doc_count" -gt 0 ]; then
            print_pass "Files found ($doc_count files)"
        else
            print_warn "Files exists but is empty (tests will use synthetic fixtures)"
        fi
    else
        print_warn "Files not found (will be created automatically)"
    fi
    ...
    local mcp_image="ghcr.io/groupdocs-annotation/annotation-net-mcp:latest"
