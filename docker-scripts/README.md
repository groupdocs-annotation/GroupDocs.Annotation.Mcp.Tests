...
Complete bash script suite for running GroupDocs.Annotation.Mcp integration tests in both local and Docker environments. **Works on Windows (Git Bash/WSL), Linux, and macOS.**

## 📋 Overview

This folder contains production-ready bash scripts for testing the GroupDocs.Annotation.Mcp NuGet package across all scenarios:
...
| `04_run-server-with-samples.sh` | **Interactive Docker MCP server** — Launches the published `annotation-net-mcp` container with `Files/` mounted, for ad-hoc smoke testing. |
...
**Output:**
```
Passed  ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsAnnotationMcp
...
services:
  annotation-server:
    image: ghcr.io/groupdocs-annotation/annotation-net-mcp:latest
    volumes:
...
| `AddAnnotation` | per-tool | `./02_test-all-scenarios.sh --filter AddAnnotation` |
| `GetAnnotations` | per-tool | `./02_test-all-scenarios.sh --filter GetAnnotations` |
| `GeneratePagesPreview` | per-tool | `./02_test-all-scenarios.sh --filter GeneratePagesPreview` |
| `ErrorHandling` | error paths | `./02_test-all-scenarios.sh --filter ErrorHandling` |
| (default) | all tests | `./02_test-all-scenarios.sh` |
...
docker build -t annotation-net-mcp:local ../../../GroupDocs.Annotation.Mcp/
...
# Docker utilities
docker_pull_image "ghcr.io/groupdocs-annotation/annotation-net-mcp:latest"
check_docker_daemon
...
- [GroupDocs.Annotation.Mcp NuGet](https://www.nuget.org/packages/GroupDocs.Annotation.Mcp)
- [Integration Tests Guide](../how-to/06-run-integration-tests.md)
- [Docker Setup Guide](../how-to/02-run-via-docker.md)
- [Test Project](../src/GroupDocs.Annotation.Mcp.Tests/GroupDocs.Annotation.Mcp.Tests.csproj)
