#!/bin/bash
################################################################################
# 04 — Run published Docker MCP image with this repo's Files mounted
#
# Launches `ghcr.io/groupdocs-annotation/annotation-net-mcp` as an interactive MCP
# server with the project's Files/ folder mounted at /data. Use this for
...
SAMPLE_DOCS_DIR="$PROJECT_ROOT/Files"
IMAGE_REPO="ghcr.io/groupdocs-annotation/annotation-net-mcp"
...
if [ ! -d "$SAMPLE_DOCS_DIR" ]; then
    log_error "Files directory not found: $SAMPLE_DOCS_DIR"
    exit 1
fi

local_count=$(find "$SAMPLE_DOCS_DIR" -type f -not -name '.gitkeep' 2>/dev/null | wc -l)
if [ "$local_count" -eq 0 ]; then
    log_warning "Files is empty — drop test files in $SAMPLE_DOCS_DIR before running"
else
...
# Mount the project's Files folder at /data so the server's default
# storage path picks them up automatically. Read-only — server writes
# nothing to /data when only running sign / non-licensed verify.
...
if [ -n "$LICENSE_PATH" ]; then
    ...
    log_info "License mounted — verify will work in licensed mode"
else
    log_info "No license — verify will return evaluation-mode error (sign works fine)"
fi
...
if [ "$RUN_MODE" = "smoke" ]; then
    ...
    if echo "$response" | grep -q '"sign"' && echo "$response" | grep -q '"verify"'; then
