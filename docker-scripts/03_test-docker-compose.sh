#!/bin/bash
################################################################################
# GroupDocs.Annotation.Mcp — Docker-Compose Integration Test Runner
#
...
#   - Files folder with test fixtures (optional)
#
# Usage:
...
#   ./03_test-docker-compose.sh --filter Sign --keep
#
################################################################################
...
TEST_PROJECT_DIR="$PROJECT_ROOT/src/GroupDocs.Annotation.Mcp.Tests"
...
validate_environment() {
    ...
    if [ ! -d "$SAMPLE_DOCS_DIR" ]; then
        print_warning "Files not found, creating empty directory"
...
create_docker_compose_file() {
    ...
    # Files is mounted RO at /Files and the fixture reads
...
services:
  test-runner:
    ...
    working_dir: /workspace/src/GroupDocs.Annotation.Mcp.Tests
    environment:
      DOTNET_NOLOGO: "true"
      DOTNET_CLI_TELEMETRY_OPTOUT: "1"
      GROUPDOCS_MCP_SAMPLE_DOCS: /Files
$license_env
    volumes:
      - $PROJECT_ROOT:/workspace
      - $SAMPLE_DOCS_DIR:/Files:ro
$license_mount
    command:
      - dotnet
      - test
...
main() {
    ...
    echo ""
    echo "╔════════════════════════════════════════════════════╗"
    echo "║  GroupDocs.Annotation.Mcp — Docker Test Runner      ║"
    echo "╚════════════════════════════════════════════════════╝"
    echo ""
