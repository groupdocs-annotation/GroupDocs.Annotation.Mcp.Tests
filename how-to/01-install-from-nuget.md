...
Two install routes from [`nuget.org/packages/GroupDocs.Annotation.Mcp`](https://www.nuget.org/packages/GroupDocs.Annotation.Mcp):
...
```bash
dnx GroupDocs.Annotation.Mcp@26.7.0 --yes
```
...
```bash
dnx GroupDocs.Annotation.Mcp --yes                # latest stable, refreshed every run
dnx GroupDocs.Annotation.Mcp --prerelease --yes   # latest including pre-releases
```
...
The cache lives at `%USERPROFILE%\.nuget\packages\groupdocs.annotation.mcp\<version>\`
on Windows and `~/.nuget/packages/groupdocs.annotation.mcp/<version>/` on
...
info: ModelContextProtocol.Server.StdioServerTransport[...]
      Server (stream) (GroupDocs.Annotation.Mcp) transport reading messages.
...
(
  ...
  sleep 2
) | GROUPDOCS_MCP_STORAGE_PATH=./docs dnx GroupDocs.Annotation.Mcp@26.7.0 --yes
```
...
```bash
dotnet tool update -g GroupDocs.Annotation.Mcp
```

To uninstall:

```bash
dotnet tool uninstall -g GroupDocs.Annotation.Mcp
```

The tool runs from `~/.dotnet/tools/groupdocs-annotation-mcp` (Linux/macOS) or
`%USERPROFILE%\.dotnet\tools\groupdocs-annotation-mcp.exe` (Windows). Make sure
...
```text
initialize response → serverInfo: { name: "GroupDocs.Annotation.Mcp", version: "26.7.0" }
```
...
| `No license configured. Running in evaluation mode.` | No `GROUPDOCS_LICENSE_PATH` | Expected. Set the path to drop the `[Evaluation mode]` prefix and the watermark on annotation output. |
| `[Evaluation mode] Output may include watermarks.` | annotation tool without license | Expected in eval mode. Set `GROUPDOCS_LICENSE_PATH` to remove watermarks. |
