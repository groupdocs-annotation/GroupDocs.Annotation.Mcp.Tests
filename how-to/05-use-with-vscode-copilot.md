...
2. Open Copilot chat.
3. Click the tools / MCP icon — `groupdocs-annotation` should appear with
   the 10 annotation tools listed (add_annotation, get_annotations, update_annotation, …).
...
The [NuGet.org package page](https://www.nuget.org/packages/GroupDocs.Annotation.Mcp)
...
```
@workspace add_annotation /docs/contract.pdf with a highlight on page 2 containing "Reviewed by Alice"

Use verify to check whether agreement.docx has any signatures, then list them.

Search /docs/invoice.pdf for QR codes and show me the decoded text.
```
