...
The `GroupDocs.Annotation.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Annotation.Mcp](https://www.nuget.org/packages/GroupDocs.Annotation.Mcp)
...
```bash
curl -L -o pkg.nupkg \
  https://www.nuget.org/api/v2/package/GroupDocs.Annotation.Mcp/26.5.0
...
```json
{
  ...
  "name": "io.github.groupdocs-annotation/groupdocs-annotation-mcp",
  "version": "26.5.0",
  "packages": [ { "registryType": "nuget", "identifier": "GroupDocs.Annotation.Mcp", ... } ],
  "repository": { "url": "https://github.com/groupdocs-annotation/GroupDocs.Annotation.Mcp" }
}
```
...
3. Search for `groupdocs-annotation` or `annotation`.
...
- `dnx` successfully resolves `GroupDocs.Annotation.Mcp@26.5.0` from nuget.org.
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-annotation"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsAnnotationMcp` — the server
  reports `serverInfo.name == "GroupDocs.Annotation.Mcp"` and a non-empty version
  that matches the package under test.
- `ToolDiscoveryTests.ListTools_ExposesReadAndVerify` — both MCP tools
  are advertised.
