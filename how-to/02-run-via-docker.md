...
- `ghcr.io/groupdocs-annotation/annotation-net-mcp` — GitHub Container Registry (primary)
- `docker.io/groupdocs/annotation-net-mcp` — Docker Hub (mirror)
...
```bash
docker pull ghcr.io/groupdocs-annotation/annotation-net-mcp:26.7.0   # pinned to exact version
docker pull ghcr.io/groupdocs-annotation/annotation-net-mcp:latest   # floats to most recent push
```
...
docker pull ghcr.io/groupdocs-annotation/annotation-net-mcp:latest
...
```bash
docker inspect ghcr.io/groupdocs-annotation/annotation-net-mcp:latest \
...
(
  ...
  sleep 2
    ...
    -v $(pwd)/documents:/data \
    ghcr.io/groupdocs-annotation/annotation-net-mcp:26.7.0 2>/dev/null
```

Expected: two JSON-RPC responses on stdout. The second lists all 10 advertised tools with their descriptions and input schemas.
...
services:
  groupdocs-annotation-mcp:
    image: ghcr.io/groupdocs-annotation/annotation-net-mcp:26.7.0
    stdin_open: true
    tty: false
    environment:
...
Without the license, the write-path tools return a `[Evaluation mode]` prefix
...
docker inspect ghcr.io/groupdocs-annotation/annotation-net-mcp:26.7.0 \
  --format '{{json .Config}}' | jq

# Expected:
# - Entrypoint: ["dotnet", "GroupDocs.Annotation.Mcp.dll"]
...
| `manifest unknown` / can't pull image | Version tag doesn't exist on that registry | Check [ghcr.io/groupdocs-annotation/annotation-net-mcp](https://github.com/groupdocs-annotation/GroupDocs.Annotation.Mcp/pkgs/container/annotation-net-mcp) for available tags. |
| `:latest` ran but didn't pick up a new release | Docker reuses cached image — `docker run :latest` does not auto-pull | `docker pull ghcr.io/groupdocs-annotation/annotation-net-mcp:latest` before running, or add `--pull always` to your `docker run`. |
